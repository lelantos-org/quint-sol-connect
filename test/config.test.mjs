...
const fixture = (m, over = {}) =>
  JSON.stringify({
    meta: {
      ...
      itf: 'fx/demo/trace-000.itf.json',
      ...over,
    },
    steps: '0xdeadbeef',
  });
...
function clean(over = {}) {
  ...
  const files = {
    'fx/demo/trace-000.json': fixture(m),
    'fx/demo/trace-000.itf.json': '{}',
  ...
  return { m, root: tmp(files) };
}

test('a consistent tree has no problems', () => {
  const { m, root } = clean();
  assert.deepEqual(checkModel(m, root), { problems: [], fixtures: 1 });
});

test('a fixture generated for another schema is named', () => {
  const { m, root } = clean({ 'fx/demo/trace-000.json': fixture(m0(), { schemaHash: `0x${'11'.repeat(32)}` }) });
...
test('a fixture generated by another tool version is named', () => {
  const { m, root } = clean({ 'fx/demo/trace-000.json': fixture(m0(), { toolVersion: '0.0.1' }) });
  assert.match(checkModel(m, root).problems.join('\n'), /generated by quint-sol-connect 0\.0\.1/);
});

test('a missing ITF companion is named', () => {
  const { m, root } = clean({ 'fx/demo/trace-000.itf.json': null });
  assert.match(checkModel(m, root).problems.join('\n'), /ITF companion .* is missing/);
});
...
test('an absent fixture directory says which command makes one', () => {
  const { m, root } = clean({ 'fx/demo/trace-000.json': null, 'fx/demo/trace-000.itf.json': null });
