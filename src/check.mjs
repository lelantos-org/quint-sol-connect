...
import { FIXTURE_FORMAT_VERSION, EXEMPLAR_ITF, emitSolidity, hashSpec } from './gen.mjs';
import { attribute, explain, humanBytes } from './budget.mjs';
import { contentHash } from './emit/solidity.mjs';
import { cap, posix } from './util.mjs';

/** Matches a generated test's fixture path, including after `forge fmt` wraps the call. */
const REPLAY_CALL = /_replay\(\s*"([^"]+)"\s*,?\s*\)/g;
 ...
 * @param {object} model
 * @param {string} root
 * @param {{ runtimeImport?: string }} [opts] the `--runtime` prefix `gen` used
 */
export function checkModel(model, root, { runtimeImport = 'quint-sol-connect' } = {}) {
  const problems = [];
  ...
  const exemplars = [];
  /** Test name and path per fixture, for re-emitting the per-trace contract. */
  const replayed = [];
  ...
  for (const f of fixtures) {
    ...
    replayed.push({ testName: meta.testName, path: posix(path.join(model.fixtureOut, model.name, f)) });

    if (meta.schemaHash !== model.schemaHash) schemaDrift.push(f);
    if (meta.formatVersion !== FIXTURE_FORMAT_VERSION) {
      const was = meta.formatVersion ?? 'none';
      formatDrift.set(was, (formatDrift.get(was) ?? 0) + 1);
    }
  ...
  for (const [version, n] of formatDrift) {
    problems.push(
      `${n} fixture(s) are in fixture format ${version}, this quint-sol-connect writes format ` +
        `${FIXTURE_FORMAT_VERSION}. The tool was bumped across an encoding change without regenerating.`,
    );
  }
  ...
  if (!fs.existsSync(tracesFile)) {
    ...
    const inTests = [...src.matchAll(REPLAY_CALL)].map((m) => m[1]);
    const relDir = posix(path.relative(root, fixtureDir));
    const expected = fixtures.map((f) => `${relDir}/${f}`);

    const missing = inTests.filter((p) => !fs.existsSync(path.resolve(root, p)));
    if (missing.length) {
      problems.push(
    ...
    const orphaned = expected.filter((p) => !inTests.includes(p));
    pathsAgree = missing.length === 0 && orphaned.length === 0;
    if (orphaned.length) {
      problems.push(
  ...
  // Re-emit the generated Solidity and compare content hashes. `forge fmt` has
  // rewritten the committed files, so bytes cannot be compared; the stamped
  // hash of the unformatted emission survives it. This covers what neither the
  // schema hash nor the fixture meta does: `pragma`, `ignoreState` reasons, the
  // runtime import, the driver contract, and a tool upgrade that changed the
  // emitted code.
  if (model.driver?.path && model.driver?.contract && replayed.length === fixtures.length) {
    for (const { file, text } of emitSolidity(model, { runtimeImport, fixtures: replayed, solDir: model.solidityOut })) {
      const abs = path.resolve(root, file);
      // Missing files and fixture/test path disagreements are named above,
      // more specifically than a hash mismatch could.
      if (!fs.existsSync(abs)) continue;
      if (file.endsWith('Traces.t.sol') && !pathsAgree) continue;
      const committed = contentHash(fs.readFileSync(abs, 'utf8'));
      if (committed !== contentHash(text)) {
        problems.push(
          `${posix(file)} ${committed ? 'differs from' : 'carries no content hash, so cannot be matched to'} ` +
            'what this config and this version of quint-sol-connect emit (pragma, ignoreState reasons, ' +
            'runtime import, driver, fixture list, or the generator itself changed)',
        );
      }
    }
  }
