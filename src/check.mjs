...
export function checkModel(model, root) {
  ...
  if (!fs.existsSync(fixtureDir)) {
    problems.push(`no fixtures at ${path.relative(root, fixtureDir)} - run \`quint-connect-sol gen ${model.name}\``);
  ...
  for (const [version, n] of versionDrift) {
    problems.push(
      `${n} fixture(s) were generated by quint-connect-sol ${version}, this is ${TOOL_VERSION}. ` +
  ...
  if (!fs.existsSync(libFile)) {
    problems.push(`${path.relative(root, libFile)} is missing - run \`quint-connect-sol gen ${model.name}\``);
  } else {
